...
export interface PlatformAbstract {
    onInit?(): void;
}

@Injectable()
@Directive()
export abstract class PlatformAbstract<T = any> implements Platform<T> {
    ...
    getOperations(route: ActivatedRouteSnapshot) {
        return route.data[FRRRI_OPERATIONS]
            ?.filter(operation => operation.platforms.includes(this.platform));
    }

    getMiddlewares() {
        return this.injector.get(FRRRI_MIDDLEWARE)
            ?.filter(middleware => middleware.supportedPlatforms.includes(this.platform));
    }
    ...
    getOperations$(route: ActivatedRouteSnapshot, state: RouterStateSnapshot) {
        ...
        operations.forEach(operation =>
            this.getMiddlewares()
                .forEach(resolver => {
                    const operate = resolver.operate(operation, this.platform, route, state);
